-- Photo Gallery v1.0 to v2.0 Database Migration
-- This script upgrades existing database schema without data loss
-- IMPORTANT: Backup your database before running this script!
-- 
-- Usage: mysql -u root -p mydb < migration_v1_to_v2.sql
-- Created: 2026-02-02
...
SELECT 
  `id`,
  SUBSTRING(`name`, 1, 500),  -- Truncate if needed
  `tags`,
  SUBSTRING(`album`, 1, 500), -- Truncate if needed
  SUBSTRING(`path`, 1, 1000), -- Truncate if needed
  CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
FROM `photos_backup`;
...
SELECT 
  `id`,
  SUBSTRING(`tag`, 1, 255),  -- Truncate if needed
  CURRENT_TIMESTAMP
FROM `tags_backup`;
...
('2.0.0', 'Migrated to v2.0: InnoDB engine, indexes, timestamps');
...
CREATE VIEW `v_photos_by_album` AS
SELECT 
  `album`,
